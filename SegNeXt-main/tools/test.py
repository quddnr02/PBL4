...
def parse_args():
    ...
    parser.add_argument('--work-dir', help='if specified, evaluation metric results will be dumped into the directory as json')
    ...
    parser.add_argument('--eval', type=str, nargs='+', help='evaluation metrics, e.g., "mIoU"')
...
def main():
    ...
    json_file = osp.join(work_dir, f"eval_{'multi_scale' if args.aug_test else 'single_scale'}_{timestamp}.json")
    dataset = build_dataset(cfg.data.test)
    loader_cfg = dict(num_gpus=len(cfg.gpu_ids), dist=distributed, shuffle=False)
    loader_cfg.update({k: v for k, v in cfg.data.items() if k not in [
    ...
    if args.format_only or eval_on_format_results:
        tmpdir = eval_kwargs.get('imgfile_prefix', '.format_cityscapes')
    ...
    if rank == 0:
        if args.out:
            warnings.warn('The behavior of ``args.out`` has been changed since MMSeg v0.16.')
